...
class AugmentedEpiduralDataset(Dataset):
    def __init__(self, subset, transform_general, transforms_epidural):
        ...
        for idx in subset.indices:
            ...
            # Ajoute 2 augmentations supplémentaires pour la classe 'epidural'
...
if __name__ == '__main__':
    ...
    lr = 1e-3
    k_folds = 5
    model_save_path = "ct_cnn_best"
    curve_save_path = "learning_curves"
    early_stopping_patience = 5  # nombre d'epochs sans amélioration avant arrêt

    # Data augmentation globale pour tout le train - évite le surapprentissage
    ...
    # Transformations spécifiques pour les épidurales - améliore la diversité des données
    ...
    # Transformations simples pour la validation
    ...
    # Chargement du dataset complet (sans transform)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(full_dataset)):
        ...
        for epoch in range(num_epochs):
            ...
            remaining = (num_epochs - (epoch + 1)) * epoch_duration

            print(f"Epoch {epoch+1}/{num_epochs} terminé en {epoch_duration:.2f}s "
                f"| Temps écoulé : {elapsed:.2f}s | Temps restant estimé : {remaining:.2f}s")

            # Sauvegarde du meilleur modèle pour ce fold
            if val_acc > best_val_acc:
                best_val_acc = val_acc
                best_epoch = epoch
                torch.save(model.state_dict(), f"{model_save_path}_fold{fold+1}.pth")
        ...
        print(f"\n Temps total d'entraînement pour le fold {fold+1} : {elapsed:.2f} secondes")
        # Tracer les courbes d'apprentissage pour ce fold
        plot_learning_curves(train_losses, val_losses, train_accs, val_accs, save_path=f"{curve_save_path}_fold{fold+1}.png")
        print(f"Meilleur modèle du fold {fold+1} sauvegardé dans {model_save_path}_fold{fold+1}")
